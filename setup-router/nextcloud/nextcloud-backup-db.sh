...
BACKUP_FILE_NAME="nextcloud-sqlbkp_$(date +"%Y%m%d").sql"
find . -name "nextcloud-sqlbkp_*.sql*" | xargs -r rm

podman run --rm \
  --security-opt seccomp=unconfined \
  --security-opt label=disable \
  --network=host \
  --mount type=bind,source=$PWD,target=/data/nextcloud \
  -e "PGPASSWORD=$NEXTCLOUD_DB_PASSWORD" docker.io/postgres:latest \
  pg_dump nextcloud -h $NEXTCLOUD_DB_HOST -U $NEXTCLOUD_DB_USER -p $NEXTCLOUD_DB_PORT -f /data/nextcloud/$BACKUP_FILE_NAME
...
cp -f "$BACKUP_FILE_NAME.tar.zst" "$NEXTCLOUD_DATA_DIR/$BACKUP_FILE_NAME.tar.zst"
find "$NEXTCLOUD_DATA_DIR" -name "nextcloud-sqlbkp_*.sql*" -ctime +100 | xargs -r rm
