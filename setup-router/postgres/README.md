...
```bash
podman exec -it postgres-db psql -h localhost -d postgres -U postgres/或其他默认用户
...
host    all     all             172.22.1.10/16                 trust" >> /var/lib/postgres-db/data/pgdata/pg_hba.conf
...
2. 修改配置文件允许内网地址连入: `/var/lib/postgres-db/data/pgdata/pg_hba.conf` (本地路由地址: 172.23.1.10/16 和 docker地址: 10.0.2.100/16) 然后 `pg_ctl reload` 。
...
# Local mode(mount)
cat sqlbkp_`date +"%Y%m%d"`.bak | podman exec -it postgres-db psql -U [username] -d [db_name]
