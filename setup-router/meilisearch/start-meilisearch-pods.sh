...
if [[ -z "$MEILISEARCH_ETC_DIR" ]]; then
  MEILISEARCH_ETC_DIR="$SCRIPT_DIR/etc"
fi
mkdir -p "$MEILISEARCH_ETC_DIR"
...
systemctl --user --all | grep -F container-meilisearch.service

if [[ $? -eq 0 ]]; then
  systemctl --user stop container-meilisearch
  systemctl --user disable container-meilisearch
fi
...
echo "[Unit]
Description=container-meilisearch
...
" | tee $MEILISEARCH_ETC_DIR/container-meilisearch.service

systemctl --user enable $MEILISEARCH_ETC_DIR/container-meilisearch.service
systemctl --user daemon-reload
systemctl --user restart container-meilisearch.service
