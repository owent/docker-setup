...
if [ $ROUTER_NET_LOCAL_ENABLE_V2RAY -ne 0 ]; then
  if [ -n "$TPROXY_SETUP_NFTABLES" ] && [ $TPROXY_SETUP_NFTABLES -ne 0 ]; then
    ROUTER_NET_LOCAL_NFTABLE_NAME=v2ray:ip:ip6:bridge
  else
    ROUTER_NET_LOCAL_IPSET_PREFIX=V2RAY
  fi
fi
if [ -n "$ROUTER_NET_LOCAL_ENABLE_VBOX" ] && [ $ROUTER_NET_LOCAL_ENABLE_VBOX -ne 0 ]; then
  if [[ -z "$ROUTER_NET_LOCAL_NFTABLE_NAME" ]]; then
    ROUTER_NET_LOCAL_NFTABLE_NAME="vbox:inet:bridge"
  else
    ROUTER_NET_LOCAL_NFTABLE_NAME="$ROUTER_NET_LOCAL_NFTABLE_NAME,vbox:inet:bridge"
  fi
fi
if [ -n "$ROUTER_NET_LOCAL_ENABLE_SDWAN" ] && [ $ROUTER_NET_LOCAL_ENABLE_SDWAN -ne 0 ]; then
  if [[ -z "$ROUTER_NET_LOCAL_NFTABLE_NAME" ]]; then
...
if [ -n "$ROUTER_NET_LOCAL_ENABLE_NAT" ] && [ $ROUTER_NET_LOCAL_ENABLE_NAT -ne 0 ]; then
  if [[ -z "$ROUTER_NET_LOCAL_NFTABLE_NAME" ]]; then
...
if [ -n "$ROUTER_NET_LOCAL_ENABLE_SECURITY" ] && [ $ROUTER_NET_LOCAL_ENABLE_SECURITY -ne 0 ]; then
  if [[ -z "$ROUTER_NET_LOCAL_NFTABLE_NAME" ]]; then
