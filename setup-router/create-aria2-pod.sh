...
save-session=/data/aria2/session/aria2.session
...
chmod 775 /data/aria2/download;
chmod 775 /data/aria2/session;

chown $RUN_USER -R "$RUN_HOME/aria2/etc" ;
chown $RUN_USER -R "$RUN_HOME/aria2/log" ;
chown $RUN_USER -R /data/aria2/download;
chown $RUN_USER -R /data/aria2/session;


cd "$(dirname "${BASH_SOURCE[0]}")";

echo '#!/bin/bash
if [ -e "/data/aria2/session/aria2.session" ]; then
    aria2c --input-file=/data/aria2/session/aria2.session "$@";
else
    aria2c "$@";
