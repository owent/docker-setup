...
KEA_EXEC_RELOAD="ExecReload=/bin/bash -c '$(which podman) kill --signal HUP kea-dhcp4'"
if [[ $FIND_PODLET_RESULT -eq 0 ]]; then
  ...
  podman generate systemd kea-dhcp4 | \
    sed "/ExecReload=/d" | \
    sed "/ExecStart=/a $KEA_EXEC_RELOAD" | \
    tee -p "$SYSTEMD_SERVICE_DIR/kea-dhcp4.service"
