...
nftables 现在已经可以在 `bridge` family 的 `prerouting` 钩子里使用 `meta broute set 1`，把原本要桥接转发的数据包改成走 L3 路由路径。
因此像 `setup-router/sdwan/setup-sdwan-route.sh` 这类脚本，可以在 bridge 入口把公网流量抬到 `inet` / `ip rule` 里继续处理，并统一复用单份 `inet#sdwan#PROXY_*` 动态集合；旧的 `ebtables -t broute ... -j redirect --redirect-target DROP` 和 `meta pkttype set unicast` 更适合作为历史兼容方案参考。
...
| xfs   | SSD      | rw,noatime,largeio,inode64,logbufs=8,logbsize=256k,noquota |
| xfs   | 网络存储 | rw,noatime,largeio,inode64,logbufs=8,swalloc,noquota       |
