...
if [[ $FIND_PODLET_RESULT -eq 0 ]]; then
  ...
  ${PODLET_RUN[@]} "${PODLET_OPTIONS[@]}" \
    podman run -d --name kea-ctrl-agent --cap-add NET_BIND_SERVICE --cap-add NET_RAW \
  ...
  podman generate systemd kea-ctrl-agent | tee -p "$SYSTEMD_SERVICE_DIR/kea-ctrl-agent.service"
